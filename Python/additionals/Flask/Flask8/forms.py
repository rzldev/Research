...
from additionals.Flask.Flask8.Models import User
...
class UpdateForm(FlaskForm) :
    ...
    def validate_username(self, username) :
        ...
        elif username.data is not current_user.username and username.data is not User.username :
            user = User.query.filter_by(username=username.data).first()
            if user :
                raise ValidationError('That username is already exist!')
    ...
    def validate_email(self, email) :
        ...
        elif email.data is not current_user.email and email.data is not User.email:
            user = User.query.filter_by(email=email.data).first()
            if user :
                raise ValidationError('That email is already exist!')
